...
Enonic builds Docker images specifically made to be used in CI/CD pipelines. Those images can be used with different CI/CD providers to create pipelines. Each image contains the newest minor version of `Enonic XP 7 or XP 8`, the newest version of the `Enonic CLI` and some other essentials for build pipelines, including Yarn and essential Node tools. The image name format is `enonic/enonic-ci:<Major Version>.<Minor Version>`:
...
Some CI/CD providers require you to set environment variables in order to deploy your application.
...
All possible environment variables are listed in the https://developer.enonic.com/docs/enonic-cli/stable/xp#environment_variables[Enonic CLI documentation].
...
. *Setup Service Account Key* https://developer.enonic.com/docs/xp/stable/iam/system-idp#service_account_key[for Enonic XP].
. *Set required Secrets* for your project https://help.github.com/en/articles/virtual-environments-for-github-actions#creating-and-using-secrets-encrypted-variables[for GitHub Actions].
    ** `url` is the address of the management port.
    ** `cred_file` is a content of Service Account Key file.
    ** `client_cert` is a content of the client certificate file - if you are using mTLS.
    ** `client_key` is a content of the client key file - if you are using mTLS.
...
. *Setup CircleCI* https://circleci.com/docs/guides/getting-started/getting-started/[for your project].
. *Set required environmental variables* for your project https://circleci.com/docs/guides/security/set-environment-variable/[for CircleCI].
...
. *Setup Travis CI* https://www.travis-ci.com/quickstart/[for your project].
...
Before we create the workflow, we first need to https://circleci.com/docs/guides/security/contexts/#create-and-use-a-context[create 3 contexts in our organization]. We will call them:
